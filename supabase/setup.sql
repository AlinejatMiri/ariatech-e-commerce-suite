-- Idempotent setup script for new Supabase project
...
-- blog_posts
DROP POLICY IF EXISTS "Published blogs public read" ON public.blog_posts;
...
-- discounts
DROP POLICY IF EXISTS "Active discounts public read" ON public.discounts;
...
-- contact_submissions
DROP POLICY IF EXISTS "Authenticated can submit contact" ON public.contact_submissions;
DROP POLICY IF EXISTS "Admin manage contact submissions" ON public.contact_submissions;
CREATE POLICY "Authenticated can submit contact" ON public.contact_submissions FOR INSERT TO authenticated WITH CHECK (true);
...
-- ========== STORAGE BUCKET ==========
-- Create product-images storage bucket (run via Supabase Dashboard if this fails)
INSERT INTO storage.buckets (id, name, public) VALUES ('product-images', 'product-images', true)
ON CONFLICT (id) DO NOTHING;

-- Storage policies: anyone can view, only admins can upload
